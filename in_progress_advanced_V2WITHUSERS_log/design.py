...
'''
def setup_ui(root, db_names, user):
    style = ttk.Style()
    ...
    tabs = {}  # Dictionary to map each tab to its database connection
    for db_name in db_names:
        tab = ttk.Frame(notebook)
        notebook.add(tab, text=db_name)
        tabs[tab] = init_db(db_name)  # Associate the tab with its database connection
        setup_tab(root, tab, db_name, user)  # Pass user as an argument

    root.tabs = tabs  # Save the tabs dictionary in the root object
    ...
    log_window = Text(log_frame, height=5, yscrollcommand=scrollbar.set, state='disabled', bg='#f0f0f0',
                      font=('Arial', 10))
    log_window.pack(side="left", fill="both", expand=True)
    scrollbar.config(command=log_window.yview)
    root.log_window = log_window  # Save the log window reference in the root object
    # Bind the tab change event to update the log window
    ...
    # Initially display logs for the first tab
    display_log(root, tabs[list(tabs.keys())[0]])  # Pass the first database connection


def update_logs_on_tab_change(event, root):
    notebook = event.widget
    selected_tab = notebook.select()  # Get the selected tab ID
    selected_tab_widget = notebook.nametowidget(selected_tab)  # Get the actual widget (Frame)
    conn = root.tabs[selected_tab_widget]  # Get the associated database connection
    display_log(root, conn)
'''
...
def setup_ui(root, db_names, user):
    ...
    log_window.pack(side="left", fill="both", expand=True)
    scrollbar.config(command=log_window.yview)
    root.log_window = log_window  # Set log_window before polling starts
