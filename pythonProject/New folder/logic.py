...
def backup_data_handler(root, conn, user):
    try:
        ...
        log_action(user[0], "Saved to Backup", conn)
    except Exception as e:
        messagebox.showerror("Error", str(e))
        log_action(user[0], f"Failed to Save Backup: {str(e)}", conn)
'''
...
'''
def display_log(root, conn):
    cursor = conn.cursor()
    cursor.execute("SELECT user_id,username,action, timestamp FROM logs ORDER BY timestamp ASC")
    logs = cursor.fetchall()

    log_window = root.log_window
    log_window.config(state='normal')
    log_window.delete(1.0, tk.END)  # Clear the log window

    for log in logs:
        user_id, username, timestamp, message = log
        log_window.insert('end', f"{timestamp}: {user_id} {username} {message}\n")
